...
class KModel(torch.nn.Module):
    ...
    def forward_with_tokens(
        ...
        if 1:
            ...
            pred_aln_trg = torch.zeros((1, input_ids.shape[1], total_frames), device=self.device)
            cur_pos = 0
            for i in range(input_ids.shape[1]):
                dur = pred_dur[i].item()
                if dur > 0:
                    end_pos = min(cur_pos + dur, total_frames)
                    pred_aln_trg[0, i, cur_pos:end_pos] = 1.0
                    cur_pos = end_pos
        ...
        print(f"pred_aln_trg: {pred_aln_trg.shape}")
