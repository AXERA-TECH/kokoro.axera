...
class SineGen_hy(nn.Module):
    ...
    def _f02sine(self, f0_values):
        ...
        # 2025-10-22 修改：改为固定全零初始化
        rand_ini = torch.zeros(f0_values.shape[0], f0_values.shape[2], 
    ...
    def forward(self, f0):
        ...
        # 2025-10-22 修改：改为固定随机种子生成的噪声

        # 直接使用形状元组
        # noise = torch.zeros(sine_waves.shape)
        # noise.normal_(0, 1)
        noise = noise_amp #* noise

        # numpy_noise = torch.normal(mean=0, std=1, size=sine_waves.shape).to(sine_waves.device)
        # noise = noise_amp * numpy_noise

        # noise = torch.zero_like(sine_waves.shape[0], sine_waves.shape[1])
        # noise.normal_(0, 1)
...
class Generator(nn.Module):
    ...
    def forward(self, x, s, f0):
        ...
        return self.forward_with_har(x, s, har)
    ...
    def forward_with_har(self, x, s, har):
        ...
        for i in range(self.num_upsamples):
            ...
            if i == self.num_upsamples - 1:
                ...
                x = F.pad(x, (1, 0), mode='constant', value=0)  # 零填充
            x = x + x_source
            xs = None
    ...
    def forward_with_har_raw(self, x, s, har):
        """
        使用预计算的 har 进行前向传播，但输出原始的 x（不经过 exp/sin/inverse）
        用于 ONNX 导出，避免量化问题
        """
        for i in range(self.num_upsamples):
            x = F.leaky_relu(x, negative_slope=0.1) 
...
class AdainResBlk1d(nn.Module):
    def __init__(self, dim_in, dim_out, style_dim=64, actv=nn.LeakyReLU(0.2), upsample='none', dropout_p=0.0):
        ...
        if upsample == 'none':
            ...
            #self.pool = weight_norm(nn.ConvTranspose1d(dim_in, dim_in, kernel_size=3, stride=2, groups=1, padding=1, output_padding=1))
...
class Decoder(nn.Module):
    def __init__(self, dim_in, style_dim, dim_out, 
        ...
        #         nn.AdaptiveAvgPool1d(1)  # 或者保持固定输出长度
        #     )
    ...
    def forward(self, asr, F0_curve, N, s):
        ...
        # import numpy as np
        # np.savetxt('x_hy.txt', x.detach().cpu().numpy().reshape(x.shape[0], -1))
        # np.savetxt('s_hy.txt', s.detach().cpu().numpy())
        # np.savetxt('F0_curve_hy.txt', F0_curve.detach().cpu().numpy())
        
        # 保留到小数点后4位
        # x = torch.round(x * 1000) / 1000
        # s = torch.round(s * 1000) / 1000
        # F0_curve = torch.round(F0_curve * 1000) / 1000
    ...
    def forward_with_har(self, asr, F0_curve, N, s, har):
        ...
        for block in self.decode:
            ...
            if block.upsample_type != "none":
                res = False
        x = self.generator.forward_with_har(x, s, har)
        return x
    
    def forward_with_har_raw(self, asr, F0_curve, N, s, har):
        """
        使用预计算的 har 进行前向传播，但输出原始的 x（不经过 exp/sin/inverse）
        用于 ONNX 导出，避免量化问题
        """
        ...
        for block in self.decode:
            ...
            if block.upsample_type != "none":
                res = False
        x = self.generator.forward_with_har_raw(x, s, har)
        return x
